resource "azurerm_management_group_policy_exemption" "exemption_definition" {
  ...
  management_group_id             = jsondecode(file("${var.file_name}")).scope
...
output "id" {
  value = azurerm_management_group_policy_exemption.exemption_definition.id
}
