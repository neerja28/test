resource "azurerm_subscription_policy_exemption" "exemption_definition" {
  ...
  subscription_id                 = jsondecode(file("${var.file_name}")).scope
...
output "id" {
  value = azurerm_subscription_policy_exemption.exemption_definition.id
}
