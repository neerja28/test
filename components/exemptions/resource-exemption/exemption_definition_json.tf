resource "azurerm_resource_policy_exemption" "exemption_definition" {
  ...
  resource_id                     = jsondecode(file("${var.file_name}")).scope
...
output "id" {
  value = azurerm_resource_policy_exemption.exemption_definition.id
}
